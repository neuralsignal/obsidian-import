...
class TestRunWithTimeoutThread:
    def test_returns_result_on_success(self):
        result = run_with_timeout(
            _return_hello, (), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="thread"
        )
        assert result == "hello"

    def test_forwards_args(self):
        result = run_with_timeout(
            "{}-{}".format, ("a", "b"), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="thread"
        )
        assert result == "a-b"
    ...
    def test_raises_timeout_when_slow(self):
        with pytest.raises(ExtractionTimeoutError, match="timed out after 1s"):
            run_with_timeout(
                _sleep_then_return, (), timeout_seconds=1, label="test", path=Path("/tmp/f.txt"), isolation="thread"
            )

    def test_raises_extraction_error_on_exception(self):
        with pytest.raises(ExtractionError, match="failed for"):
            run_with_timeout(
                _raise_value_error, (), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="thread"
            )
    ...
    def test_raises_extraction_error_on_none_result(self):
        ...
        with pytest.raises(ExtractionError, match="returned no result"):
            run_with_timeout(
                _return_none, (), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="thread"
            )

    def test_error_message_includes_label_and_path(self):
        with pytest.raises(ExtractionTimeoutError, match="PDF") as exc_info:
            run_with_timeout(
                _sleep_then_return, (), timeout_seconds=1, label="PDF", path=Path("/data/doc.pdf"), isolation="thread"
            )
    ...
    def test_timeout_message_includes_file_size_mb(self, tmp_path: Path):
        ...
        with pytest.raises(ExtractionTimeoutError) as exc_info:
            run_with_timeout(_sleep_then_return, (), timeout_seconds=1, label="PDF", path=big, isolation="thread")
    ...
    def test_preserves_original_exception_as_cause(self):
        with pytest.raises(ExtractionError) as exc_info:
            run_with_timeout(
                _raise_runtime_error, (), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="thread"
            )
    ...
    def test_unknown_isolation_mode_raises_config_error(self):
        with pytest.raises(ConfigError, match="isolation"):
            run_with_timeout(
                _return_hello, (), timeout_seconds=5, label="test", path=Path("/tmp/f.txt"), isolation="fiber"
            )
    ...
    def test_thread_mode_leaks_worker_thread_after_timeout(self, tmp_path: Path):
        ...
        with pytest.raises(ExtractionTimeoutError):
            run_with_timeout(_blocked, (), timeout_seconds=1, label="test", path=path, isolation="thread")
